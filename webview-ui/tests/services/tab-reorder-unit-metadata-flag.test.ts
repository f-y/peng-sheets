...
describe('Category A: metadataRequired MUST be false', () => {
     ...
     */
    // Note: Classifier returns physical+metadata when docs are present (SS swap needs metadata for display order)
    it('S1 after S2 with docs after WB (proper index)', () => {
        ...
        // SS swap with docs: classifier returns physical+metadata
        // Physical move-sheet + metadata to maintain display order
    ...
    it('toIndex=2 - S1 after S2 with single doc after WB', () => {
        ...
        // SS swap with doc: classifier returns physical+metadata
        expect(action.actionType).toBe('physical+metadata');
    });
    ...
    it('toIndex=1 - S1 after S2 without docs', () => {
        ...
        // SS swap without docs: toIndex=1 from index=0 is same position (no-op)
    ...
    describe('A2: Sheet swap with docs on both sides', () => {
         ...
         */
        // D1 before WB needs metadata to stay first in display order
        it('S1 after S2 with docs on both sides', () => {
            ...
            // D1 is before WB physically, needs metadata to maintain display order
            expect(action.actionType).toBe('physical+metadata');
        });
    });
    ...
    describe('A4: Doc-to-doc movement (same side of WB)', () => {
         ...
         */
        // Without physicalStructure, classifier uses visual order as physical
        // DD swap returns metadata when no explicit structure provided
        it('D1 after D2 (both after WB) - metadata without physicalStructure', () => {
            const tabs: TestTab[] = [
                { type: 'sheet', sheetIndex: 0 },
                { type: 'sheet', sheetIndex: 1 },
                { type: 'document', docIndex: 0 },
...
describe('Category B: metadataRequired MUST be true', () => {
    ...
    describe('B2: Last sheet to after doc (C8v)', () => {
         ...
         */
        // H9 pattern triggers - D1 becomes visually first, so move-workbook needed
        it('C8v: Last sheet to after doc - H9 triggers physical+metadata', () => {
            const tabs: TestTab[] = [
                { type: 'sheet', sheetIndex: 0 },
                { type: 'sheet', sheetIndex: 1 },
                { type: 'document', docIndex: 0 },
            ...
            // H9 triggers: D1 becomes first, requires physical+metadata
            expect(action.actionType).toBe('physical+metadata');
            expect(action.metadataRequired).toBe(true);
        });
    });
    ...
    describe('B3: Doc moves between sheets (D6/D7)', () => {
         ...
         */
        // Doc moves between sheets needs physical+metadata
