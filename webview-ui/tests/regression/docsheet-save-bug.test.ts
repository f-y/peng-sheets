...
`;
    ...
    it.skip('BUG DOCUMENTATION: two updateRange messages cause content to be prepended', () => {
        ...
        // Simulate VS Code applying second update
        ...
        // Apply second update
        const finalContent = fileContent.substring(0, 0) + secondUpdate.content + fileContent.substring(endOffset);
        console.log('Final content:', JSON.stringify(finalContent));

        // BUG: 'before' should NOT be in the final content!
        if (finalContent.includes('before')) {
            console.log('BUG REPRODUCED: "before" still in content!');
        }
