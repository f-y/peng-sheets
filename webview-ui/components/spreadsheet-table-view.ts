...
export class SpreadsheetTableView extends LitElement {
    ...
    private _getColumnTemplate(colCount: number): string {
        // Use hardcoded values like Container does
        // 30px for row header, 100px default column width
        const widths: string[] = ['30px']; // Row header
            ...
            ${filterMenu
                ? html`
                      <filter-menu
                          ...
                          @filter-apply="${(e: CustomEvent<{ selectedValues: Set<string> }>) => {
                        this.dispatchEvent(
                            new CustomEvent('view-filter-apply', {
                                detail: { col: filterMenu.col, selectedValues: e.detail.selectedValues },
                                bubbles: true,
                                composed: true
                            })
                        );
                          ...
                          @filter-close="${() => {
                        this.dispatchEvent(
                            new CustomEvent('view-filter-close', { bubbles: true, composed: true })
                        );
                    }}"
                      ></filter-menu>
                  `
